...
if not SIM_FLAG:
    ...
    while True:
        ...
        if len(datarecv) > 0:
            ...
            srcEdgeNumList = (msg.split(';')[2]).split(',');
    ...
    if AGENT_TYPE == "multi_agent":
        ...
        for i in range(AGENT_NUM):
            if(agents[i] == None):
                ...
                reward = 0.
                if RWD_SELE[0] == "1":
                    reward += np.sum(np.log(np.array(thr_sum)))
                if RWD_SELE[1] == "1":
                    reward += np.sum(-DELTA * np.log(np.array(delay_sum)))
                if RWD_SELE[2] == "1":
                    reward += np.sum(-DELTA * np.log(np.array(ecnpkt) + 1e-5))
                #reward = np.sum(np.log(np.array(thr_sum)) - DELTA * np.log(np.array(delay_sum)))
                #reward = np.sum(np.log(np.array(thr_sum)))
                #reward = -np.sum(np.log(np.array(ecnpkt) + 1e-10))
                #reward = np.sum(- DELTA * np.log(np.array(delay_sum)))
        ...
        return multi_state_new, multi_reward, multi_thr, multi_delay
    else:
        for i in range(sessionNum):
            pacNos.append([])
            dels.append([])
    ...
    if AGENT_TYPE == "multi_agent":
        ...
        for i in range(AGENT_NUM):
            if agents[i] != None:
                ...
                ret_c_t.append(agents[i].predict(state[i], reward[i], thr[i], dly[i]))
            else:
                ret_c_t.append([])
        for i in range(len(sessionSrc)):
        ...
        ret_c = tuple(agents[0].predict(state[0], reward[0], thr[0], dly[0]))
    else:
        # for OSPF and MCF
        ret_c = action
